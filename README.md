...
**[Very Important- Please read this paragraph italicized and given below before using this app**  


*As a pro tip which has recently been tested and found to be working, Login to Instagram on Firefox or any other browser using the same account you are using for download in the app and just do some activity like liking any post before opening this app and trying to download as well as after finishing with your downloads. This should avoid Instagram identifying that you are using an app to grab its contents. Also in the rare case of instagram not allowing you to login in the app (failed login even after entering correct username and password with no challenge error raised in app), unfortunately you have no other option but to manually install instagrapi(explained below), download the DesktopApplication folder given here and then run app.py to login and get the session.js file saved. This session file can then be moved to the installation directory of your app( usually on Program files on Windows) to make your app working perfectly like before. Over the past few months, I found this app to be extremely useful for my needs and one day when I wanted to grab all the stories associated with an account, it took just 2 sec to fill the folder on my lap with all the 70 or so stories. The app is in 100% working condition and does everything exactly like I said here and there is nothing else like this at the momemt, but again unfortunately, Iam finding Instagram to be very very over strict when it comes to scraping and so problems keep coming. Therefore I request you to create an alternate fake account just for using this app, so that you wont lose anything if Insta blocks your access permanently.*  
...
Install instagrapi which which works using Instagram's official API for fetching content so you don't have to worry much about authorisation issues
